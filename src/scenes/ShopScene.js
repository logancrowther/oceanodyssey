...
const CATCH_DRAWERS = {
  ...
  dragonfish: (g, x, y, weightKg) => drawDragonfish(g, x, y, 0.76 * sizeScaleFor('dragonfish', weightKg)),
  fangtooth: (g, x, y, weightKg) => drawFangtooth(g, x, y, 0.72 * sizeScaleFor('fangtooth', weightKg)),
  black_dragonfish: (g, x, y, weightKg) => drawBlackDragonfish(g, x, y, 0.6 * sizeScaleFor('black_dragonfish', weightKg)),
  viperfish: (g, x, y, weightKg) => drawViperfish(g, x, y, 0.65 * sizeScaleFor('viperfish', weightKg)),
  pacific_blackdragon: (g, x, y, weightKg) => drawPacificBlackdragon(g, x, y, 0.7 * sizeScaleFor('pacific_blackdragon', weightKg)),
  loosejaw: (g, x, y, weightKg) => drawLoosejaw(g, x, y, 0.6 * sizeScaleFor('loosejaw', weightKg)),
  stoplight_loosejaw: (g, x, y, weightKg) => drawStoplightLoosejaw(g, x, y, 0.6 * sizeScaleFor('stoplight_loosejaw', weightKg)),
  // A much smaller base multiplier than every other icon here - the real
  // animal's own extremely long ribbon body would otherwise overflow a
  // normal-sized row at the same scale everything else uses.
  giant_oarfish: (g, x, y, weightKg) => drawGiantOarfish(g, x, y, 0.28 * sizeScaleFor('giant_oarfish', weightKg)),
  giant_grenadier: (g, x, y, weightKg) => drawGiantGrenadier(g, x, y, 0.55 * sizeScaleFor('giant_grenadier', weightKg)),
  roundnose_grenadier: (g, x, y, weightKg) => drawRoundnoseGrenadier(g, x, y, 0.6 * sizeScaleFor('roundnose_grenadier', weightKg)),
  coryphaenoid_grenadier: (g, x, y, weightKg) =>
    drawCoryphaenoidGrenadier(g, x, y, 0.6 * sizeScaleFor('coryphaenoid_grenadier', weightKg)),
  abyssal_grenadier: (g, x, y, weightKg) => drawAbyssalGrenadier(g, x, y, 0.6 * sizeScaleFor('abyssal_grenadier', weightKg)),
  tripod_fish: (g, x, y, weightKg) => drawTripodFish(g, x, y, 0.6 * sizeScaleFor('tripod_fish', weightKg)),
  telescopefish: (g, x, y, weightKg) => drawTelescopefish(g, x, y, 0.6 * sizeScaleFor('telescopefish', weightKg)),
  barreleye_fish: (g, x, y, weightKg) => drawBarreleyeFish(g, x, y, 0.6 * sizeScaleFor('barreleye_fish', weightKg)),
  deep_sea_hatchetfish: (g, x, y, weightKg) =>
    drawDeepSeaHatchetfish(g, x, y, 0.54 * sizeScaleFor('deep_sea_hatchetfish', weightKg)),
  giant_hatchetfish: (g, x, y, weightKg) => drawGiantHatchetfish(g, x, y, 0.6 * sizeScaleFor('giant_hatchetfish', weightKg)),
  bigeye_fish: (g, x, y, weightKg) => drawBigeyeFish(g, x, y, 0.65 * sizeScaleFor('bigeye_fish', weightKg)),
  deep_sea_smelt: (g, x, y, weightKg) => drawDeepSeaSmelt(g, x, y, 0.58 * sizeScaleFor('deep_sea_smelt', weightKg)),
  bristlemouth: (g, x, y, weightKg) => drawBristlemouth(g, x, y, 0.5 * sizeScaleFor('bristlemouth', weightKg)),
...
const BAIT_ICON_DRAWERS = {
  ...
  deep_sea_bait: (g, x, y) => drawDeepSeaBait(g, x, y, 1.7),
  colossal_bait: (g, x, y) => drawColossalBait(g, x, y, 1.9),
...
// genuine step up from that, Shimmering Lure more again, and Abyssal
// Bait (the Mythic pull) a true rarity - about 1 in 200 crates.
const CRATE_COST = 1000;
const CRATE_MYTHIC_CHANCE = 0.005;
const CRATE_LEGENDARY_CHANCE = 0.02;
const CRATE_EPIC_CHANCE = 0.06;
const CRATE_RARE_CHANCE = 0.145;
const CRATE_UNCOMMON_CHANCE = 0.25;
// (the remaining ~0.52 is the common payout below)

function rollCrate() {
  const r = Math.random();
  if (r < CRATE_MYTHIC_CHANCE) {
    return { tier: 'mythic', itemId: 'abyssal_bait', qty: 1 };
  }
  if (r < CRATE_MYTHIC_CHANCE + CRATE_LEGENDARY_CHANCE) {
    return { tier: 'legendary', itemId: 'shimmering_lure', qty: 1 };
  }
  if (r < CRATE_MYTHIC_CHANCE + CRATE_LEGENDARY_CHANCE + CRATE_EPIC_CHANCE) {
    return { tier: 'epic', itemId: 'colossal_bait', qty: 1 };
  }
  if (r < CRATE_MYTHIC_CHANCE + CRATE_LEGENDARY_CHANCE + CRATE_EPIC_CHANCE + CRATE_RARE_CHANCE) {
    return { tier: 'rare', itemId: 'plastic_lure', qty: 1 };
  }
  if (r < CRATE_MYTHIC_CHANCE + CRATE_LEGENDARY_CHANCE + CRATE_EPIC_CHANCE + CRATE_RARE_CHANCE + CRATE_UNCOMMON_CHANCE) {
    return { tier: 'uncommon', itemId: 'deep_sea_bait', qty: 5 };
  }
...
const CRATE_STRIP_POOL = [
  ...
  { tier: 'uncommon', itemId: 'deep_sea_bait' },
  { tier: 'rare', itemId: 'plastic_lure' },
  { tier: 'epic', itemId: 'colossal_bait' },
  { tier: 'legendary', itemId: 'shimmering_lure' },
  { tier: 'mythic', itemId: 'abyssal_bait' }
];
const CRATE_STRIP_WEIGHTS = [0.3, 0.3, 0.18, 0.1, 0.06, 0.04, 0.02];
