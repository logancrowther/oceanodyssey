...
import {
  ...
  drawBlackDragonfish,
  drawViperfish,
  drawPacificBlackdragon,
  drawLoosejaw,
  drawStoplightLoosejaw,
  drawGiantOarfish,
  drawGiantGrenadier,
  drawRoundnoseGrenadier,
  drawCoryphaenoidGrenadier,
  drawAbyssalGrenadier,
  drawTripodFish,
  drawTelescopefish,
  drawBarreleyeFish,
  drawDeepSeaHatchetfish,
  drawGiantHatchetfish,
  drawBigeyeFish,
  drawDeepSeaSmelt,
  drawBristlemouth,
  drawGreatWhite,
...
const DRAWERS = {
  ...
  black_dragonfish: drawBlackDragonfish,
  viperfish: drawViperfish,
  pacific_blackdragon: drawPacificBlackdragon,
  loosejaw: drawLoosejaw,
  stoplight_loosejaw: drawStoplightLoosejaw,
  giant_oarfish: drawGiantOarfish,
  giant_grenadier: drawGiantGrenadier,
  roundnose_grenadier: drawRoundnoseGrenadier,
  coryphaenoid_grenadier: drawCoryphaenoidGrenadier,
  abyssal_grenadier: drawAbyssalGrenadier,
  tripod_fish: drawTripodFish,
  telescopefish: drawTelescopefish,
  barreleye_fish: drawBarreleyeFish,
  deep_sea_hatchetfish: drawDeepSeaHatchetfish,
  giant_hatchetfish: drawGiantHatchetfish,
  bigeye_fish: drawBigeyeFish,
  deep_sea_smelt: drawDeepSeaSmelt,
  bristlemouth: drawBristlemouth,
...
const BAIT_HOOK_SCALE = {
  ...
  dragonfish: 0.32,
  fangtooth: 0.4,
  black_dragonfish: 0.3,
  viperfish: 0.34,
  pacific_blackdragon: 0.4,
  loosejaw: 0.3,
  stoplight_loosejaw: 0.3,
  giant_oarfish: 0.6,
  giant_grenadier: 0.5,
  roundnose_grenadier: 0.42,
  coryphaenoid_grenadier: 0.4,
  abyssal_grenadier: 0.4,
  tripod_fish: 0.36,
  telescopefish: 0.34,
  barreleye_fish: 0.34,
  deep_sea_hatchetfish: 0.28,
  giant_hatchetfish: 0.36,
  bigeye_fish: 0.4,
  deep_sea_smelt: 0.3,
  bristlemouth: 0.26,
...
const REVEAL_SCALE = {
  ...
  dragonfish: 0.7,
  fangtooth: 0.85,
  black_dragonfish: 0.62,
  viperfish: 0.68,
  pacific_blackdragon: 0.78,
  loosejaw: 0.6,
  stoplight_loosejaw: 0.6,
  giant_oarfish: 1.0,
  giant_grenadier: 0.9,
  roundnose_grenadier: 0.8,
  coryphaenoid_grenadier: 0.78,
  abyssal_grenadier: 0.78,
  tripod_fish: 0.72,
  telescopefish: 0.66,
  barreleye_fish: 0.66,
  deep_sea_hatchetfish: 0.55,
  giant_hatchetfish: 0.7,
  bigeye_fish: 0.78,
  deep_sea_smelt: 0.6,
  bristlemouth: 0.52,
  great_white: 1.3,
...
const SWIM_SCALE = {
  ...
  stoplight_loosejaw: 0.3,
  giant_oarfish: 0.6,
  giant_grenadier: 0.48,
  roundnose_grenadier: 0.4,
  coryphaenoid_grenadier: 0.38,
  abyssal_grenadier: 0.38,
  tripod_fish: 0.36,
  telescopefish: 0.32,
  barreleye_fish: 0.32,
  deep_sea_hatchetfish: 0.28,
  giant_hatchetfish: 0.34,
  bigeye_fish: 0.38,
  deep_sea_smelt: 0.3,
  bristlemouth: 0.26,
  great_white: 1.15,
...
const SWIM_SPEED = {
  ...
  black_dragonfish: 16,
  viperfish: 18,
  pacific_blackdragon: 17,
  loosejaw: 15,
  stoplight_loosejaw: 15,
  giant_oarfish: 20,
  giant_grenadier: 22,
  roundnose_grenadier: 20,
  coryphaenoid_grenadier: 20,
  abyssal_grenadier: 19,
  tripod_fish: 12,
  telescopefish: 17,
  barreleye_fish: 15,
  deep_sea_hatchetfish: 16,
  giant_hatchetfish: 17,
  bigeye_fish: 18,
  deep_sea_smelt: 20,
  bristlemouth: 18,
  great_white: 48,
...
const DEPTH_LIMITS = {
  ...
  // The rest of the Abyss roster (see ABYSS_ONLY_SPECIES below) - every
  // one of these only ever turns up past 1000m, spread out across the
  // 1000-3000m range so going progressively deeper keeps turning up new
  // species rather than everything unlocking at once at the boundary.
  giant_oarfish: { min: 12000 },
  bigeye_fish: { min: 12000 },
  deep_sea_smelt: { min: 12000 },
  bristlemouth: { min: 12000 },
  black_dragonfish: { min: 13200 },
  tripod_fish: { min: 13200 },
  viperfish: { min: 14400 },
  deep_sea_hatchetfish: { min: 14400 },
  pacific_blackdragon: { min: 15600 },
  roundnose_grenadier: { min: 15600 },
  loosejaw: { min: 16800 },
  giant_hatchetfish: { min: 16800 },
  coryphaenoid_grenadier: { min: 18000 },
  telescopefish: { min: 20400 },
  giant_grenadier: { min: 21600 },
  barreleye_fish: { min: 24000 },
  abyssal_grenadier: { min: 36000 },
  old_boot: { max: 1800 }
};
...
// past it except the species that actually belong to the Abyss
// (ABYSS_ONLY_SPECIES below) and the Kraken (whose own eligibility check
// is separate and untouched by this boundary). This is a hard stop, not a
// depth gate like DEPTH_LIMITS' own `min`/`max` - even Abyssal Bait/Hook's
// usual gate-bypass never invents a new max, so it doesn't get past this
// either.
const ABYSS_BOUNDARY = 12000;
const ABYSS_ONLY_SPECIES = new Set([
  'dragonfish',
  'fangtooth',
  'angler_fish',
  'black_dragonfish',
...
// snag). A compact horizontal bar: hold the mouse/touch down to push the
// player's own bar forward (right), release it to let it drift back
// (left); the fish's bar wanders left-right on its own within the same
...
export default class OceanScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.minigameHintText = this.add
      .text(0, 0, 'Hold to Reel!', label('13px', { color: '#bfe9ff' }))
      .setOrigin(0.5, 1)
      .setVisible(false);
  ...
  startMinigame(f) {
    ...
    this.minigame = {
      fish: f,
      playerX: MINIGAME_TRACK_LEFT + playerBarW / 2,
      fishX: Phaser.Math.Between(MINIGAME_TRACK_LEFT + 30, MINIGAME_TRACK_RIGHT - 30),
      fishTargetX: Phaser.Math.Between(MINIGAME_TRACK_LEFT + 30, MINIGAME_TRACK_RIGHT - 30),
      fishSpeed,
      fishRetargetMs,
      ...
      drainRate,
      // Whether the player has ever actually held the button down yet -
      // gates the "Hold to Reel!" hint (see redrawMinigame), which only
      // ever needs to show once, right at the start.
      everHeld: false
    };
  }
  ...
  updateMinigame(deltaMs) {
    ...
    if (held) m.everHeld = true;
    m.playerX += (held ? MINIGAME_PLAYER_FORWARD_PX : -MINIGAME_PLAYER_BACK_PX) * dt;
    // Clamped so the bar's own edges stay inside the track, not just its
    // center point - otherwise it visibly pokes out past either end
    // whenever it's pinned at an extreme.
    m.playerX = Phaser.Math.Clamp(
      m.playerX,
      MINIGAME_TRACK_LEFT + m.playerBarW / 2,
      MINIGAME_TRACK_RIGHT - m.playerBarW / 2
    );
    ...
    if (m.fishRetargetTimer <= 0 || Math.abs(m.fishX - m.fishTargetX) < 4) {
      m.fishTargetX = Phaser.Math.Between(MINIGAME_TRACK_LEFT + 20, MINIGAME_TRACK_RIGHT - 20);
      m.fishRetargetTimer = m.fishRetargetMs;
    }
    const fishDir = Math.sign(m.fishTargetX - m.fishX);
    m.fishX += fishDir * m.fishSpeed * dt;

    const overlapping = Math.abs(m.fishX - m.playerX) <= m.playerBarW / 2;
  ...
  finishMinigame(success) {
    ...
    this.minigame = null;
    this.minigameHintText.setVisible(false);
    if (success) {
      this.catchFish(f);
    } else {
      this.landedCatch = null;
  ...
  redrawMinigame() {
    ...
    g.clear();
    const m = this.minigame;
    if (this.phase !== 'minigame' || !m) {
      this.minigameHintText.setVisible(false);
      return;
    }

    const trackY = MINIGAME_TRACK_Y;
    const trackLeft = MINIGAME_TRACK_LEFT;
    const trackLen = MINIGAME_TRACK_RIGHT - MINIGAME_TRACK_LEFT;
    const th = MINIGAME_TRACK_THICKNESS;
    const r = th / 2;

    // Track background - a full pill shape rather than a plain rounded
    // rect, so both ends read as smooth caps instead of square corners.
    g.fillStyle(0x02121c, 0.78);
    g.fillRoundedRect(trackLeft, trackY - r, trackLen, th, r);
    g.lineStyle(2, 0x0c3446, 0.95);
    g.strokeRoundedRect(trackLeft, trackY - r, trackLen, th, r);

    const overlapping = Math.abs(m.fishX - m.playerX) <= m.playerBarW / 2;

    // A soft pulsing glow behind the player's bar while it's actually
    // sitting on the fish - the clearest possible "you're doing it right"
    // signal, on top of the colour swap below.
    if (overlapping) {
      const pulse = 0.3 + Math.sin(this.waveT * 10) * 0.15;
      g.fillStyle(0x4ad991, pulse);
      g.fillRoundedRect(m.playerX - m.playerBarW / 2 - 6, trackY - r - 6, m.playerBarW + 12, th + 12, r + 6);
    }
    ...
    const innerH = th - 8;
    g.fillStyle(overlapping ? 0x4ad991 : 0x4fb8e8, 0.9);
    g.fillRoundedRect(m.playerX - m.playerBarW / 2, trackY - innerH / 2, m.playerBarW, innerH, innerH / 2);
    g.lineStyle(2, overlapping ? 0x2e8a5e : 0x2b6f94, 1);
    g.strokeRoundedRect(m.playerX - m.playerBarW / 2, trackY - innerH / 2, m.playerBarW, innerH, innerH / 2);

    // The fish's own marker - a soft layered glow behind a bright core,
    // the same "glow" convention the rest of this game's own art already
    // uses (see tackle.js), drawn on top of the player's bar so it never
    // disappears underneath it.
    g.fillStyle(0xffb74a, 0.22);
    g.fillCircle(m.fishX, trackY, 11);
    g.fillStyle(0xffb74a, 0.45);
    g.fillCircle(m.fishX, trackY, 8);
    g.fillStyle(0xffb74a, 1);
    g.fillCircle(m.fishX, trackY, 5);
    g.lineStyle(1.5, 0x7a4a10, 0.9);
    g.strokeCircle(m.fishX, trackY, 5);

    // The progress meter, above the track - fills as the player keeps
    // landing on the fish, drains when they miss it.
    const pbX = DESIGN_WIDTH / 2;
    const pbY = trackY - 55;
    const pbW = 190;
    const pbH = 14;
    const pbR = pbH / 2;
    g.fillStyle(0x02121c, 0.78);
    g.fillRoundedRect(pbX - pbW / 2, pbY, pbW, pbH, pbR);
    g.fillStyle(0x4ad991, 0.95);
    g.fillRoundedRect(pbX - pbW / 2 + 2, pbY + 2, Math.max(0, (pbW - 4) * m.progress), pbH - 4, pbR - 2);
    g.lineStyle(2, 0x0c3446, 0.95);
    g.strokeRoundedRect(pbX - pbW / 2, pbY, pbW, pbH, pbR);

    // A brief "Hold to Reel!" hint the first time - gone for good the
    // moment the player actually holds down once.
    this.minigameHintText.setPosition(pbX, pbY - 20);
    this.minigameHintText.setVisible(!m.everHeld);
  }
